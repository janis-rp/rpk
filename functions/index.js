...
// v2 onCall (atstāj kā ir)
const { onCall, HttpsError } = require('firebase-functions/v2/https');

// ✅ Gen1 (v1) priekš Auth onCreate (darbojas uzreiz)
...
// ────────────────────────────
// Auth onCreate (Gen1 / v1)
// ────────────────────────────
exports.authOnCreate = functionsV1.auth.user().onCreate(async (user) => {
  const ref = db.collection('parent').doc(user.uid);
  ...
  await ref.set(
    {
      ...
      lastName: last || null,
      fullName: display || [first, last].filter(Boolean).join(' ') || null,
...
exports.mergeParentData = onCall({ region: 'us-central1' }, async (request) => {
  ...
  if (
    !userRecord ||
    normalizePhone(userRecord.phoneNumber || '') !== normalized
  ) {
    throw new HttpsError(
      'failed-precondition',
      'Phone not verified for this user.',
    );
  }
  ...
  const patch = {
    phoneVerified: true,
    legacy: { matched: matches > 0, matches },
  };
  if (best) {
    if (!current.displayName && (best.vards || best.uzvards))
      patch.displayName = [best.vards, best.uzvards].filter(Boolean).join(' ');
    ...
    if (!current.personalCode && best.personaskods)
      patch.personalCode = best.personaskods;
  }
 ...
 */
exports.adminUnlinkPhone = onCall(
  { region: 'us-central1' },
  async (request) => {
    ...
    await db
      .collection('users')
      .doc(uid)
      .set({ phone: null, phoneVerified: false }, { merge: true });
...
exports.completeMerge = onCall({ region: 'us-central1' }, async (req) => {
  ...
  if (!sourceUid)
    throw new HttpsError('invalid-argument', 'Missing sourceUid.');
  if (sourceUid === targetUid)
    throw new HttpsError('failed-precondition', 'Same account.');

  // 1) Merge intent
  const intentRef = db.collection('mergeIntents').doc(sourceUid);
  const intentSnap = await intentRef.get();
  if (!intentSnap.exists)
    throw new HttpsError('failed-precondition', 'Merge intent not found.');
  ...
  if (!targetPhone)
    throw new HttpsError(
      'failed-precondition',
      'Target user has no verified phone.',
    );
  ...
  // 5) Sapludinām users/B ar TRŪKSTOŠAJIEM laukiem no A
  const patchDst = {};
